...
public class ClientJsonWorker implements Runnable, IObserver {
    ...
    private Response handleRequest(Request request) {
        ...
        if (request.getType() == RequestType.GET_ALL_GAMES_FOR_PLAYER) {
            System.out.println("Get all games for player request..." + request.getType());
            Player player = request.getPlayer();
            try {
                return JsonProtocolUtils.createGetAllGamesForPlayerResponse(server.getAllGamesForPlayer(player));
            } catch (AppException e) {
                return JsonProtocolUtils.createErrorResponse(e.getMessage());
            }
        }
