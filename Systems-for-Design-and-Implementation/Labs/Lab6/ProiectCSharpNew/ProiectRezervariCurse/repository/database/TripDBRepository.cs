...
namespace ProiectRezervariCurse.repository.database
{
        ...
        using (IDbCommand command = connection.CreateCommand())
        {
            ...
            using (IDataReader result = command.ExecuteReader())
            {
                while (result.Read())
                {
                    ...
                    string destination = result.GetString(1);
                    string departureTimeString = result.GetString(2);
                    DateTime departureTime = DateTime.ParseExact(departureTimeString, "yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture);
                    long numberOfSeats = result.GetInt64(3);
                    Trip trip = new Trip(destination, departureTime, numberOfSeats);
                    trip.Id = id;
                    trips.Add(trip);
