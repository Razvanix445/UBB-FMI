...
public class TransportServicesImpl implements ITransportServices {
    ...
    public synchronized ReservationManager login(ReservationManager reservationManager, ITransportObserver client) throws TransportException {
    ...
    public synchronized Trip[] getAllTrips() throws TransportException {
        ...
        System.out.println("Got all trips from Impl: ");
        for (Trip trip: trips)
    ...
    public synchronized Reservation addReservation(Reservation reservation) throws TransportException {
    ...
    public synchronized List<Reservation> getReservationsByTrip(Trip trip) throws TransportException {
        Iterable<Reservation> reservations = reservationRepository.findAll();
        List<Reservation> result = new ArrayList<>();
        for (Reservation reservation : reservations) {
            if (reservation.getTrip().getId().equals(trip.getId())) {
    ...
    public synchronized List<Seat> getSeatsByTrip(Trip trip) throws TransportException {
        //return StreamSupport.stream(seatRepository.findSeatsByTripId(tripId).spliterator(), false).toList();
        List<Seat> seats = StreamSupport.stream(seatRepository.findSeatsByTripId(trip.getId()).spliterator(), false).toList();
        for (Seat seat: seats)
            System.out.println(seat);
        return seats;
    }

    public synchronized ReservationManager logout(ReservationManager reservationManager, ITransportObserver client) throws TransportException {
