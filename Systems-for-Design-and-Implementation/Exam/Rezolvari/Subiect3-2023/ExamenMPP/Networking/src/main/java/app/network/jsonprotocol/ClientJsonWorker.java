...
public class ClientJsonWorker implements Runnable, IObserver {
    ...
    private Response handleRequest(Request request) {
        ...
        if (request.getType() == RequestType.ADD_CONFIGURATION) {
            System.out.println("Add configuration request..." + request.getType());
        ...
        if (request.getType() == RequestType.ADD_CONFIGURATION_WORD) {
            System.out.println("Add configurationWord request..." + request.getType());
            ConfigurationWord configurationWord = request.getConfigurationWord();
            try {
        ...
        if (request.getType() == RequestType.FIND_PLAYER_BY_ALIAS) {
            ...
            try {
                return JsonProtocolUtils.createFindPlayerByAliasResponse(server.findPlayerByAlias(alias));
            } catch (AppException e) {
                return JsonProtocolUtils.createErrorResponse(e.getMessage());
            }
        }
        if (request.getType() == RequestType.GET_ALL_CONFIGURATIONS) {
            System.out.println("Get all configurations request..." + request.getType());
            try {
                return JsonProtocolUtils.createGetAllConfigurationsResponse(server.getAllConfigurations());
            } catch (AppException e) {
                return JsonProtocolUtils.createErrorResponse(e.getMessage());
            }
        }
        if (request.getType() == RequestType.GET_ALL_WORDS) {
            System.out.println("Get all words request..." + request.getType());
            try {
                return JsonProtocolUtils.createGetAllWordsResponse(server.getAllWords());
            } catch (AppException e) {
                return JsonProtocolUtils.createErrorResponse(e.getMessage());
            }
        }
