...
*/
vector<Film> Service::filtreazaDupaGen(const string& anotimp) {
	vector<Film> pesti = repository.getAll();
	vector<Film> pestiFiltrati;

	auto iterator = std::copy_if(pesti.begin(), pesti.end(), std::back_inserter(pestiFiltrati), [&](const Film& peste) {
		return peste.getGenFilm().find(anotimp) != std::string::npos;
		});
...
const vector<Film>& Service::stergeFilmDinCosInchirieri(const string& titluFilm) {
	this->cosInchirieri.stergeFilmDinCosInchirieri(titluFilm);
	return this->cosInchirieri.getAllCosInchirieri();
}
