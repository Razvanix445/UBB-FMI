...
public class FriendRequestDBRepository implements FriendRequestRepository {
    ...
    @Override
    public Page<FriendRequest> findAll(Pageable pageable, Long userId) {
        Set<FriendRequest> friendRequestSet = new HashSet<>();
        String selectSQL = "SELECT * FROM FriendRequests WHERE fromUserId = ? OR toUserId = ? ORDER BY id OFFSET ? LIMIT ?";
        try (Connection connection = DriverManager.getConnection(url, username, password);
             PreparedStatement statement = connection.prepareStatement(selectSQL);
        ) {
            ...
            return new PageImplementation<>(pageable, friendRequestSet.stream());
        } catch (SQLException e) {
            throw new RuntimeException("Error finding friend request.", e);
        }
    }
