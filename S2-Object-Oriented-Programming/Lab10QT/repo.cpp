...
void Repo::modificaFilm(const Film& filmModificat) {
	...
	for (auto& film: filme)
		if (film.getTitluFilm() == filmModificat.getTitluFilm()) {
			film.setGenFilm(filmModificat.getGenFilm());
			film.setAnAparitie(filmModificat.getAnAparitie());
			film.setActorPrincipal(filmModificat.getActorPrincipal());
		}
}
