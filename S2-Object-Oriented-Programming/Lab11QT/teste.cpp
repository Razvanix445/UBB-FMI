...
void testExportaFilmeFisierFunctionalitati() {
	...
	//verificam daca functioneaza corect adaugarea, stergerea si modificarea in fisier
	//adaugare:
	service.adaugaFilm("John Wick: Chapter 4", "Actiune", 2023, "Keanu Reeves");
	service.adaugaFilm("The Imitation Game", "Historic Drama", 2014, "Benedict Cumberbatch");
	service.adaugaFilm("Titanic", "Epic Romance", 1997, "Leonardo DiCaprio");
	...
	RepoFile repositoryFile2{ "filmetest.txt" };
	//verificam daca datele au fost citite corect
	assert(repositoryFile2.getAll().size() == 2);
	assert(repositoryFile2.getAll()[0].getTitluFilm() == "Star Wars: Episode IV");
	assert(repositoryFile2.getAll()[0].getGenFilm() == "Science Fiction");
	assert(repositoryFile2.getAll()[0].getAnAparitie() == 1977);
	assert(repositoryFile2.getAll()[0].getActorPrincipal() == "Carrie Fisher");

	assert(repositoryFile2.getAll()[1].getTitluFilm() == "Inception");
	assert(repositoryFile2.getAll()[1].getGenFilm() == "Science Fiction");
	assert(repositoryFile2.getAll()[1].getAnAparitie() == 2010);
	assert(repositoryFile2.getAll()[1].getActorPrincipal() == "Leonardo DiCaprio");

	std::ofstream fileout("filmetest.txt", std::ios::trunc);
	fileout.close();
}
...
void testUndo() {
	...
	service.modificaFilm("John Wick: Chapter 4", "Drama", 2007, "Arnold Scwarzenegger");
	assert(repositoryFile.getAll().size() == 1);
	service.stergeFilm("John Wick: Chapter 4");
	assert(repositoryFile.getAll().size() == 0);

	service.undo();
	assert(repositoryFile.getAll().size() == 1);
	assert(repositoryFile.getAll()[0].getTitluFilm() == "John Wick: Chapter 4");
	assert(repositoryFile.getAll()[0].getGenFilm() == "Drama");
	assert(repositoryFile.getAll()[0].getAnAparitie() == 2007);
	assert(repositoryFile.getAll()[0].getActorPrincipal() == "Arnold Scwarzenegger");

	service.undo();
	assert(repositoryFile.getAll().size() == 1);
	assert(repositoryFile.getAll()[0].getTitluFilm() == "John Wick: Chapter 4");
	assert(repositoryFile.getAll()[0].getGenFilm() == "Actiune");
	assert(repositoryFile.getAll()[0].getAnAparitie() == 2023);
	assert(repositoryFile.getAll()[0].getActorPrincipal() == "Keanu Reeves");

	service.undo();
	assert(repositoryFile.getAll().size() == 0);
}

void test_GetAll_ValidVectorOfFilme_RepositoryFake() {
	RepositoryFake repoFake(2);
	Validator validator;
	CosInchirieriFilme cosInchirieri;
	Service service{ repoFake, validator, cosInchirieri };

	vector<Film> filmeFiltrate = service.filtreazaDupaAnAparitie(2020);

	assert(filmeFiltrate.size() == 2);
}

void test_GetAll_InvalidVectorOfFilme_RepositoryFake() {
	RepositoryFake repoFake(1);
	Validator validator;
	CosInchirieriFilme cosInchirieri;
	Service service{ repoFake, validator, cosInchirieri };
...
void ruleazaTesteRepo() {
	...
	testExportaFilmeFisierFunctionalitati();
	testReadAllFromFile();
}
...
void ruleazaToateTestele() {
	...
	test_GetAll_ValidVectorOfFilme_RepositoryFake();
	test_GetAll_InvalidVectorOfFilme_RepositoryFake();
}